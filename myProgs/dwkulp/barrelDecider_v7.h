...
{ 
    ...
    bool rootConnect(int last,int v, int root, bool visited[], vector<int> path, vector<int> &_globalPath); // Recursive function to check whether root connects back to root
    bool rootBarrel(int root, vector<int> &_globalPath); // Whether a barrel exist from current root
...
public: 
    ...
    bool isBarrel(vector<int> &_globalPath); // Return true is is_barrel
